...
pub fn collect_tvs(ty: &ast::Type, loc: &ast::Loc, tvs: &mut OrderMap<Id, Option<Kind>>) {
    match ty {
        ast::Type::Named(named_ty) => {
            collect_named_ty_fvs(named_ty, loc, tvs);
        }
        ...
        ast::Type::Variant {
            ...
            for alt in alts {
                collect_named_ty_fvs(alt, loc, tvs);
            }
            if let Some(ext) = extension {
...
fn collect_named_ty_fvs(ty: &ast::NamedType, loc: &ast::Loc, tvs: &mut OrderMap<Id, Option<Kind>>) {
    let ast::NamedType { name, args } = ty;
