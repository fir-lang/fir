...
pub(super) fn convert_ast_ty(tys: &TyMap, ast_ty: &ast::Type, loc: &ast::Loc) -> Ty {
    match ast_ty {
        ...
        ast::Type::Variant {
            ...
            for ty in alts {
                let named_ty = convert_named_ty(tys, ty, loc);
                let old = ty_alts.insert(ty.name.clone(), named_ty);
                assert!(old.is_none());
            }
...
fn convert_named_ty(tys: &TyMap, ast_ty: &ast::NamedType, loc: &ast::Loc) -> Ty {
    let ty_con = tys
        .get_con(&ast_ty.name)
        .unwrap_or_else(|| panic!("{}: Unknown type {}", loc_display(loc), ast_ty.name));

    if ty_con.arity() as usize != ast_ty.args.len() {
        panic!(
    ...
    if ast_ty.args.is_empty() {
        return Ty::Con(ty_con.id.clone(), Kind::Star);
    }

    let converted_args: Vec<Ty> = ast_ty
        .args
        .iter()
